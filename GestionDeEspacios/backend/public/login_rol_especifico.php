...
if (empty($data['identificador']) || empty($data['password']) || empty($data['rol'])) {
    echo json_encode(['success' => false, 'message' => 'Usuario, contraseña y rol son requeridos']);
    exit;
}

$identificador = trim($data['identificador']);
$password = $data['password'];
$rol_seleccionado = $data['rol'];

$conn = getDBConnection();

// Buscar usuario por nombre de usuario o correo electrónico con el rol específico
$stmt = $conn->prepare("
    SELECT 
        u.id_usuario,
        u.rut_numero,
...
AND r.nombre_rol = ?
");
$stmt->bind_param('sss', $identificador, $identificador, $rol_seleccionado);
$stmt->execute();
...
if ($result->num_rows > 0) {
    ...
    if (password_verify($password, $usuario['contrasena_hash'])) {
        ...
        $sesion_data = [
            'action' => 'crear',
            'tipo_usuario' => strtolower($rol_seleccionado),
...
echo json_encode(['success' => false, 'message' => 'Usuario, contraseña o rol incorrectos']);
exit;
?>
